...
from util.mamba3d_utils import misc
from util.mamba3d_utils.checkpoint import get_missing_parameters_message, get_unexpected_parameters_message
from util.mamba3d_utils.logger import *
import random
from knn_cuda import KNN
from util.mamba3d_extensions.chamfer_dist import ChamferDistanceL1, ChamferDistanceL2
...
###ordering
import math
from model.mamba3d.z_order import *
...
class Mamba3D(nn.Module):
    ...
    def forward(self, pts):  # torch.Size([1, 3, 1024])

        pts = pts.transpose(1, 2).contiguous() # torch.Size([1, 1024, 3])
        neighborhood, center = self.group_divider(pts) # B G K 3  torch.Size([1, 128, 32, 3])  torch.Size([1, 128, 3])
        group_input_tokens = self.encoder(neighborhood)  # B G C  torch.Size([1, 128, 384])
        ...
        x = self.norm(x) # torch.Size([1, 129, 384])
        concat_f = torch.cat([x[:, 0], x[:, 1:].max(1)[0] + x[:, 1:].mean(1)[0]], dim=-1) # torch.Size([1, 768])
