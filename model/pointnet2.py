...
class PointNetSetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        if self.group_all:
            ...
            new_xyz, new_points = sample_and_group( # torch.Size([1, 512, 3]) torch.Size([1, 512, 32, 3])
...
class PointNet2ClsSsg(nn.Module):
    ...
    def forward(self, xyz):
        B, _, _ = xyz.shape # torch.Size([1, 3, 1024])
        l1_xyz, l1_points = self.sa1(xyz, None) # torch.Size([1, 3, 512]) torch.Size([1, 128, 512])
        l2_xyz, l2_points = self.sa2(l1_xyz, l1_points) # torch.Size([1, 3, 128]) torch.Size([1, 256, 128])
        l3_xyz, l3_points = self.sa3(l2_xyz, l2_points) # torch.Size([1, 3, 1]) torch.Size([1, 1024, 1])
        x = l3_points.view(B, 1024)
        fea_fig = x
        x = self.drop1(F.relu(self.bn1(self.fc1(x))))
        x = self.drop2(F.relu(self.bn2(self.fc2(x))))
